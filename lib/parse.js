...
const PRICE_SIZE = /(?:[-–—:]\s*)(\d+(?:\.\d+)?\s*(?:mg|g|kg|ml|l|oz|lb|pcs?|units?))\s*$/i;
const INLINE_UNIT_PRICE = /^\s*\$?\s*(\d{1,3}(?:,\d{3})+|\d+)(\.\d{1,2})?\s*(?:per|\/)\s*([a-z][a-z0-9]*)\s*$/i;
...
function findHeaderColumns(grid) {
  ...
  for (let r = 0; r < limit; r++) {
    ...
    for (let c = 0; c < grid[r].length; c++) {
      ...
      if (PRICE_HEADER.test(header)) {
        const size = header.match(PRICE_SIZE);
        priceCols.push({ index: c, unit: size ? size[1].replace(/\s+/g, '') : '' });
      }
    }
...
function extractSpreadsheet(buffer, filename) {
  ...
  for (const sheetName of wb.SheetNames) {
    ...
    for (let r = startRow; r < grid.length; r++) {
      ...
      // Each populated size column is a distinct purchasable catalog item.
      // Include the size in its name so later imports and order matching do not
      // collapse 100g, 500g, and 1kg prices into the same database row.
      let foundPrice = false;
      for (const { index: priceCol, unit } of priceCols) {
        ...
        if (cents == null || cents <= 0) continue;
        rows.push({ name: unit ? `${name} (${unit})` : name, price_cents: cents });
        foundPrice = true;
      }

      // Some vendor sheets place a lone "$1,800 per gram" value in a notes
      // column. Accept only that exact price-per-unit shape, and only when no
      // declared price column was populated for the row.
      if (!foundPrice) {
        for (let c = 0; c < row.length; c++) {
          if (c === nameCol) continue;
          const inline = String(row[c] == null ? '' : row[c]).match(INLINE_UNIT_PRICE);
          if (!inline) continue;
          const cents = toCents((inline[1] || '') + (inline[2] || ''));
          if (cents != null && cents > 0) {
            rows.push({ name: `${name} (${inline[3].toLowerCase()})`, price_cents: cents });
            break;
          }
        }
      }
